...
`analyze_data` `analyze_image` `analyze_pdf` `transcribe` `meeting_notes` · `make_slides`(G) `make_html_report`(G) `rephrase` ·
`maps` `open_app`(G) `clipboard` `notify` `location` `speak` · `shell`(G) `git_sync`(G) `request_approval`.
